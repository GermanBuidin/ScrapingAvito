...
class Scraper:
    ...
    def get_information_about_cars(self, number: int) -> list:
        links = self.get_cars_links(number)
        cars = []
        logger.info('Начало работы с карточками машин')
        for number, link in enumerate(links):
            tree = self.request_handler(link)
    ...
    def main(self, number: int):
        ...
        car_info = self.get_information_about_cars(number)
...
if __name__ == '__main__':
    number_pages = int(input('введите количество страниц поиска: '))
    Start = Scraper()
    Start.main(number_pages)
